...
#endif
{
	...
	TxBufferPtr = (data_t *)TX_BUFFER_BASE ;
	RxBufferPtr = (data_t *)RX_BUFFER_BASE;
	 ...
	 * is enabled
	 */
	Xil_DCacheFlushRange((UINTPTR)TxBufferPtr, MAX_PKT_LEN*sizeof(data_t));
	Xil_DCacheFlushRange((UINTPTR)RxBufferPtr, MAX_PKT_LEN*sizeof(data_t));

	for (Index = 0; Index < Tries; Index ++) {


		Status = XAxiDma_SimpleTransfer(&AxiDma, (UINTPTR) RxBufferPtr,
						MAX_PKT_LEN*sizeof(data_t), XAXIDMA_DEVICE_TO_DMA);
		...
		Status = XAxiDma_SimpleTransfer(&AxiDma, (UINTPTR) TxBufferPtr,
						MAX_PKT_LEN*sizeof(data_t), XAXIDMA_DMA_TO_DEVICE);
...
static int CheckData(void)
{
	...
	RxPacket = (data_t *) RX_BUFFER_BASE;
	Value = TEST_START_VALUE;

	/* Invalidate the DestBuffer before receiving the data, in case the
	 * Data Cache is enabled
	 */
	Xil_DCacheInvalidateRange((UINTPTR)RxPacket, MAX_PKT_LEN*sizeof(data_t));
	xil_printf("Rx/Tx \r\n");
	for (Index = 0; Index < MAX_PKT_LEN; Index++) {
		xil_printf("Data %i/%i \r\n",RxPacket[Index],
				   Value);
		if (RxPacket[Index] !=  255 - Value) {
			xil_printf("Data error %d: %i/%i\r\n",
